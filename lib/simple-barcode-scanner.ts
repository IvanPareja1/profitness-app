...
class UltraBarcodeScannerPro {
    ...
    try {
      ...
      if ('BarcodeDetector' in window) {
        try {
          ...
          console.warn('⚠️ Error inicializando BarcodeDetector:', error);
        }
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      const tryLoadScript = () => {
        ...
        const timeoutId = setTimeout(() => {
          script.remove();
          console.warn(`⏱️ Timeout cargando ZXing desde: ${cdnUrls[currentIndex]}`);
          currentIndex++;
          tryLoadScript();
        }, loadTimeout);
  ...
  stop(): void {
    ...
    if (this.zxingReader) {
      try {
        ...
        console.warn('⚠️ Error al detener ZXing reader:', error);
      }
    }
  ...
  // ... resto de métodos mantienen la misma lógica pero optimizada ...
...
// ... resto del código de APIs de productos mantiene las mejoras anteriores pero optimizado ...
  ...
  try {
    ...
    const promises = apis.map(async (api) => {
      ...
      try {
        ...
        clearTimeout(timeoutId);
        console.warn(`⚠️ Error con ${api.name}:`, error);
        return null;
      }
    });
...
function generateEnhancedProductImage(barcode: string, productName?: string): string {
  ...
  return `https://readdy.ai/api/search-image?query=$%7BencodeURIComponent%28query%29%7D&width=200&height=200&seq=barcode_${barcode}&orientation=squarish`;
}
...
// ... resto de funciones mantienen las mejoras pero optimizadas ...
